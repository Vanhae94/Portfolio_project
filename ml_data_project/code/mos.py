...
rain = pd.read_csv("./data/(일별)서울강수량.csv", encoding="cp949")
hum = pd.read_csv("./data/(일별)서울시습도.csv", encoding="cp949")
...
mos_weather_data = mos_weather.drop(['종합모기지수','날짜','지점','모기예보 단계'],axis=1)
mos_weather_lable = mos_weather.pop('모기예보 단계')

# # 평기 최저기 최고기 강수 평습 -> data / 종합모기 -> 레이블 === 회귀데이터
# mos_weather_data = mos_weather.drop(['종합모기지수','날짜','지점'],axis=1)
# mos_weather_lable = mos_weather.pop('종합모기지수')
# print(mos_weather_data)
# print(mos_weather_lable)

# print(mos_weather_data.info())

# 주요도가 높은 특성 세개만 남김
#mos_weather_data = mos_weather_data.drop(['최고기온(℃)','강수량(mm)','평균습도(%rh)','평균기온(℃)'], axis=1)
print(mos_weather_data)
print(mos_weather_lable)
...
# print("선형 훈련 정확도 평가 점수:", linear_model.score(x_train, y_train))
# print("선형 테스트 정확도 평가 점수 : ", linear_model.score(x_test, y_test) )
...
# print("훈련 세트 정확도: {:.3f}".format(hgb_model.score(x_train, y_train)))
# print("테스트 세트 정확도: {:.3f}".format(hgb_model.score(x_test, y_test)))

# from sklearn.inspection import permutation_importance
# n_features = mos_weather_data.shape[1]
...
# 특성 중요도 추출 (히그부)
# result = permutation_importance(hgb_model,x_train,y_train,n_repeats=10, random_state=42,n_jobs=-1)
...
# gbt_model = GradientBoostingRegressor(random_state=70, n_estimators=80, max_depth=2)
# gbt_model.fit(x_train,y_train)
# print("훈련 세트 정확도: {:.3f}".format(gbt_model.score(x_train, y_train)))
# print("테스트 세트 정확도: {:.3f}".format(gbt_model.score(x_test, y_test)))
...
# rf_model = RandomForestClassifier() # n_estimators=200, random_state=18, max_features='sqrt'
# #cross_scores = cross_val_score(rf_model,mos_weather_data,mos_weather_lable)
...
# }
# grid_search = GridSearchCV(estimator=rf_model, param_grid=grid_param, cv=5, scoring='accuracy')
...
# print("베스트모델 정확도 :", best_rf_model.score(x_test,y_test))
# print("베스트모델 정확도 accuracy 활용 :", accuracy_score(y_test,rf_y_pred))

# 교차검증
#print('교차검증:', cross_scores)

# # SVC
# from sklearn.svm import SVC
# svc_model = SVC(C=1,gamma=0.1).fit(x_train,y_train)
#
# 그래디언트 부스팅 분류
from sklearn.ensemble import GradientBoostingClassifier
gbc_model = GradientBoostingClassifier(random_state=18, max_features= 'sqrt') #  n_estimators=100, max_depth=2, learning_rate=0.15
...
# print("조정된 정확도 그부",rf_model.score(x_test_scaled,y_test))
#print("조정된 정확도 랜포",rf_model.score(x_test_scaled,y_test))
# print("svc점수",svc_model.score(x_test_scaled,y_test))
'''
...
# 25년도 기후 회귀모델로 예측
mos_weather25 = gbr25.predict(mos_weather23)
