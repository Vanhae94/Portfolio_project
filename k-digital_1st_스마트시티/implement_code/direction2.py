...
model = YOLO("bestyolo2.pt")  # 학습된 가중치 파일 경로로 변경

# 영상 파일 로드
video_path = r"D:\cctv_project\video\video_1.mp4"  # 입력 영상 파일 경로
cap = cv2.VideoCapture(video_path)
output_path = r"D:\cctv_project\video\저장\이동예측5.mp4"  # 출력 영상 파일 경로
...
while cap.isOpened():
    ...
    for track_id, (prev_x, prev_y) in previous_positions.items():
        if track_id not in tracks[:, -1]:  # 현재 추적 ID에 포함되지 않은 경우
            ...
            #cv2.circle(frame, (future_x, future_y), 5, (0, 0, 0), -1)  # 예측 위치 표시
