...
from app.models import User, CCTV, DetectionLog, Setting
from app.model_py.route import generate_frames_route
from app.model_py.map import generate_frames_heatmap
from datetime import datetime
from app.utils import generate_frames
...
@main.route('/video-stream/<cctv_id>/<modelType>')
def video_stream(cctv_id, modelType):
    try:
    ...
    # 모델 유형에 따른 함수 매핑
    model_mapping = {
        "route": generate_frames_route,  # 경로 추적 스트리밍 함수
        "map": generate_frames_heatmap   # 밀집도 히트맵 스트리밍 함수
    }

    if modelType in model_mapping:
        # 해당 모델의 프레임 제너레이터를 선택
        frame_generator = model_mapping[modelType](device_index, cctv_id)

        # 이미 스트리밍이 가능한 상태로 처리되므로 바로 Response로 반환
        return Response(
            stream_with_context(frame_generator),  # 직접 스트리밍
            mimetype='multipart/x-mixed-replace; boundary=frame'  # 응답 타입 설정
        )
    else:
